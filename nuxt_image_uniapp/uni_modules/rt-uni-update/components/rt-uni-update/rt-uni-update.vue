...
export default { 
	data() {
		return {
			version: versionName,//当前运行版本(打包时manifest里的版本名称)
			percent: 0, //进度条百分比
	...
	onLoad({obj}) {
		...
		this.data.describe = data.content.replace(/\n/g, '<br>');
		this.data.edition_name = data.newVersion	
		this.data.edition_force = Number(data.isForceUpdate)
		this.data.package_type = data.isHotUpdate
		this.data.edition_url =  platform == 'android' ?  data.pkgUrl : data.iosUrl
